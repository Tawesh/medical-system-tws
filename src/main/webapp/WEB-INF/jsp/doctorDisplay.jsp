    ...
    <div class="row">
        ...
        <div class="col-12 col-lg-10">
            <form action="findDoctor" method="get">
                <label>
                    <input type="text" name="name" class="form-control" placeholder="请输入医生姓名"/>
                </label>
            ...
            <div class="jumbotron" style="overflow-y: scroll; max-height: 70vh;">
                <table class="table table-borderless" id="table_info">
                    <thead class="table-light">
                    <tr>
                        <th scope="col">序号</th>
                        <th scope="col">医生编号</th>
                        <th scope="col">医生姓名</th>
                        <th scope="col">医生年龄</th>
                        <th scope="col">医生性别</th>
                    ...
                    <tbody>
                    <c:forEach items="${doctorList}" var="s" varStatus="num">
                        <tr>
                            <td>${num.index+1}</td>
                            <td>${s.getId()}</td>
                            <td>${s.getName()}</td>
                            ...
                            <td>${s.getPhone()}</td>
                            <td>${s.getRoom()}</td>
                            <td>${s.getWorkStartTime()}</td>
                            <td>${s.getWorkEndTime()}</td>
                            <td>
...
<script>
    function del(id, name) {
        ...
        if (b) {
            window.location = "deleteDoctor?id=" + id;
        }
    }

    function add() {
        window.location = "addDoctorPage";
    }

    function edit(id){
        window.location = "editDoctor?id="+id
    }

function printDoc(){
        window.location = "printDocPage"
}
